...
double CBSCommonFunc::perdictFlyTime(VertexPtr& now, VertexPtr& future) {
	...
	future->heading = zc::detect::FuncCommon::angle_standard(future->heading);
	double angle1 = zc::detect::FuncCommon::point_angle(now->x, now->y, future->x, future->y);
	angle1 = zc::detect::FuncCommon::angle_standard(angle1);
